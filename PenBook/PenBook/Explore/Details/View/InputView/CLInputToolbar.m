...
//    [self addSubview:self.topLine];
    //底部线条
    self.bottomLine = [[UIView alloc] initWithFrame:CGRectMake(27.5, self.CLheight - 1, kMainScreenWidth - 55, 1)];
    self.bottomLine.backgroundColor = UIColorFromRGB(0xECECEC);
...
//    [self addSubview:self.edgeLineView];
    ...
    self.textView.CLwidth = self.CLwidth - 46;
    self.textView.CLleft = 30;
    ...
    self.placeholderLabel.CLleft = 30;
    [self addSubview:self.placeholderLabel];
    //发送按钮
    self.sendButton = [[UIButton alloc] initWithFrame:CGRectMake(self.CLwidth - 52 - 25, self.CLheight - 26 - 10, 52, 26)];
    self.sendButton.backgroundColor = UIColorFromRGB(0x5061FB);
...
-(void)setFontSize:(CGFloat)fontSize{
    ...
    self.CLheight = ceil(lineH) + 10 + 10 + 180;
...
//        [self.sendButton setTitleColor:RGBACOLOR(0, 0, 0, 1.0) forState:UIControlStateNormal];
    ...
    self.CLheight = ceil(textView.CLheight) + 10 + 10 + 180;
...
//    self.edgeLineView.CLheight = self.textView.CLheight + 10;
    self.textView.CLcenterY = self.CLheight * 0.45;
    self.placeholderLabel.CLheight = self.textView.CLheight;
    self.placeholderLabel.CLcenterY = self.CLheight * 0.45;
    
    self.sendButton.CLy = self.textView.CLtop - 61;
//    self.edgeLineView.CLcenterY = self.CLheight * 0.5;
    self.bottomLine.CLy = self.textView.CLbottom + 10;
}
